...
export default function Layout() {
  ...
  return (
    <div className="flex min-h-screen">
      {/* Mobile Header */}
      <div className="fixed top-0 left-0 right-0 h-16 glass border-b border-white/10 flex items-center justify-between px-4 z-40 md:hidden">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-lg bg-electric-blue flex items-center justify-center">
            <Zap className="w-4 h-4 text-white" />
          </div>
          <span className="font-heading text-sm font-bold uppercase tracking-wider text-white">Authority</span>
        </div>
        <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
          ...
          <SheetContent side="left" className="w-64 p-0 glass border-r border-white/10 flex flex-col">
      ...
      <aside className="hidden md:flex fixed left-0 top-0 h-screen w-64 glass border-r border-white/10 flex-col z-50">
        <SidebarContent />
      </aside>
